...
const DrawerMenu = (props: Props) => {
    ...
    Linking.canOpenURL(emailUrl)
      .then((supported) => {
        if (!supported) {
          ...
          Alert.alert('Error', 'No se puede abrir la aplicación de correo en este dispositivo.');
        } else {
      ...
      .catch((error) => {
        ...
        Alert.alert('Error', 'Ocurrió un problema al intentar abrir la aplicación de correo.');
      });
  }
  ...
  return (
    <DrawerContentScrollView style={styles.drawerMain} contentContainerStyle={styles.scrollContent}>
      <SafeAreaView style={styles.main}>
        <View style={styles.header}>
          <Pressable onPress={() => props.navigation.closeDrawer()}>
            <Image style={styles.headerIcon} source={x_close_icon} />
          </Pressable>
          <Image style={styles.headerAppIcon} source={app_com_logo} resizeMethod="resize" />
          <Image style={styles.headerRightIcon} source={x_close_icon} />
        </View>

        <View style={styles.rowButtonsContainer}>
          <Pressable style={styles.buttonContainer} onPress={handleGoToLanguagesSettings}>
            <Image style={styles.flagIcon} source={languageSelected.icon} />
            <TextComponent size="16" color="muted" weight="light" transform2="uppercase">
              {`${languageSelected.country}`}
            </TextComponent>
            <Image style={styles.rightIcon} source={chevron_right} />
          </Pressable>

          <Pressable style={styles.buttonContainer} onPress={handleGoToCurrenciesSettings}>
        ...
        <View style={styles.contentList}>
          ...
          <Pressable style={styles.manuItem} onPress={goToTermsAndConditionsScreen}>
            ...
            <TextComponent size="16" color="dark">{t('drawer-terms')}</TextComponent>
          </Pressable>

          <Pressable style={styles.manuItem} onPress={goToPrivacyPolices}>
            <Image source={dual_screen_outline_icon} style={styles.contentIcon} />
            <TextComponent size="16" color="dark">{t('drawer-privacy')}</TextComponent>
          </Pressable>

          <Pressable style={styles.manuItem}>
            <Image source={dual_screen_outline_icon} style={styles.contentIcon} />
            <TextComponent size="16" color="dark">{t('drawer-legal')}</TextComponent>
          </Pressable>
        </View>
        ...
        {isLoggedIn && (
          <View style={styles.contentList}>
            <Pressable style={styles.manuItem} onPress={handleLogout}>
              ...
              <TextComponent size="16" color="dark">{t('drawer-logout')}</TextComponent>
            </Pressable>
          </View>
        )}
      </SafeAreaView>
