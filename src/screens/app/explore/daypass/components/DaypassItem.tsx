...
const DaypassItem = (props: Props) => {
	...
	useFocusEffect(
		useCallback(
			() => {
				const p = purchaseTickets.find(
					ticket => ticket.id_venue_ticket === props.venueTicket.id_venue_ticket,
				);
				if (p) {
	...
	const ServiceIcon =
		servicesIconsIndex[props.venueTicket.id_venue_ticket] || servicesIconsIndex["1"];
				...
				<View style={styles.content}>
					...
					<View style={styles.mainContent}>
						<TextComponent size="14" color="dark">
							{/* {`${currencySelected.symbol}${formatToCurrency(
								props.venueTicket.adult_fare,
							)}`} */}
							{`${formatToCurrency(
								props.venueTicket.adult_fare,
