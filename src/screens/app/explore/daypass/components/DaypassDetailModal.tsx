...
import BaseModalFullFill, { BaseModalFullFillProps } from '@app/components/molecules/modals/BaseModalFullFill';
      ...
      <View style={styles.header}>
        ...
        <View style={styles.titleContainer}>
          ...
          {searchFilterData?.date_search && (
            <TextComponent size="14" weight="semibold" color="muted" textAlign="center">
              {formatDate(parseDateStringToDate(searchFilterData.date_search), 'D MMM YYYY')}
            </TextComponent>
          )}
        </View>
        <View />
      ...
      <ScrollView style={styles.scrollMain} contentContainerStyle={styles.scrollContent}>
        ...
        <SectionItems title={t('features_and_amenities')}>
          {hotelVenueSelectedTicket.ticket_general_services.map((ticket, index) => {
            const ServiceIcon = servicesIconsIndex[ticket.id_genserv] ?? servicesIconsIndex['1'];
            return (
              <LabelWithIcon
      ...
      <BottomContainer showMargin style={styles.bottomContainer}>
        <View>
          <TextComponent color="primary" size="16" weight="bold">
            {`${searchFilterData?.currency ?? 'USD'} $${formatToCurrency(calculateTotalAmount())}`}
          </TextComponent>
        </View>
        <ButtonComponent
          title={t('add')}
