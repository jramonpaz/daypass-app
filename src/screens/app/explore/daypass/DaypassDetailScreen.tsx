import { ActivityIndicator, Alert, ScrollView, Share, StatusBar, StyleSheet, View } from 'react-native';
...
import { hotelActions, userPurchaseCheckService, viewer_venueGetDetailService, viewer_venueGetReviewsService, viewer_venueGetTicketsService } from '@app/store/slices/hotels';
...
import { IGetVenueDetailsRequest, IGetVenueReviewsRequest, IPurchaseVenueCheckPayload, IVenueTicketsRequest } from '@app/types';
...
const DaypassDetailScreen = (props: Props) => {
  ...
  const loadData = useCallback(
    () => {
      if (!hotelVenueDetail && hotelVenueTickets.length === 0 && hotelSelected && searchSelectedPrediction && searchFilterData) {
    ...
    [dispatch, hotelSelected, searchFilterData, searchSelectedPrediction, hotelVenueDetail, hotelVenueTickets, currency, language],
  );
  ...
  function totalAmount () {
    const total = purchaseTickets.reduce((acc, ticket) => (
      ticket.fare + acc
    ), 0);
    return total;
  }
  ...
  return (
    <View style={styles.scroll}>
      ...
      <BottomContainer showMargin>
        <View>
          <TextComponent color="muted" size="14">{t('total-price')}</TextComponent>
          <TextComponent color="primary" size="16" weight="bold">
            {`${searchFilterData?.currency ?? 'USD'} ${currencySelected.symbol}${formatToCurrency(totalAmount())}`}
          </TextComponent>
        </View>
        <ButtonComponent
