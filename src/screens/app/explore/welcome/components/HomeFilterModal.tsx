...
import { IAPIExplorerSearch, IGeneralServiceFilter, IMoodFilter, IVibesFilter } from "@app/types";
...
const HomeFilterModal = (props: HomeFilterModalProps) => {
	...
	const { searchFilterData, searchFilterDataBackup, searchSelectedPrediction } = useAppSelector(
		state => state.search,
	);
	...
	useEffect(() => {
		...
		if (isActive) {
			...
			count += generalServicesList.filter(service => service.isSelected).length;
	...
	useFocusEffect(
		useCallback(() => {
			...
			if (generalServicesList.length === 0) {
				dispatch(general_getAllGeneralService());
			}
		}, [dispatch, generalServicesList, ticketsMoodList, vibesList]),
	);

	function handleSelectGeneralService(service: IGeneralServiceFilter, value: boolean) {
		dispatch(exploreActions.selectOneGeneralService({ service, active: value }));
		// refreshCountData();
	...
	const buildFilterData = (getMinMax: boolean = getMinMaxFilter): IAPIExplorerSearch | null => {
		...
		const selectedGgeneralServices = generalServicesList.filter(
			service => !!service.isSelected,
		);

		//console.log("Selected General Services:", selectedGgeneralServices);
		...
		return {
			...
			ticket_genserv: selectedGgeneralServices.map(service => service.id_genserv).join(","),
		};
	};
	...
	useFocusEffect(
		useCallback(
			...
			[hotelStarTypes, ticketsMoodList, vibesList, generalServicesList],
		),
	);
	...
	return (
		<BaseModalFullFill {...props}>
			...
			<ScrollView
					...
					{ticketsMoodList.map(mood => (
						<CheckboxLabel
							...
							onChange={(value: boolean) =>
								handleSelectTicketMood(mood, value)
							}
						/>
					))}
				</SectionItems>
				...
				{/* Servicios */}
				<SectionItems title={t("filter_modal-general_services")}>
					{generalServicesList.map(service => {
						return (
							<View
								key={`${service.id_genserv}-${service.name}`}
								style={{
									flexDirection: "row",
									gap: 10,
								}}>
								<CheckboxLabel
									...
									key={`${service.id_genserv}-${service.name}`}
									onChange={(value: boolean) =>
										handleSelectGeneralService(service, value)
									}
								/>
								<View
									style={{
									...
									<ServiceIcon
										uri={generateServiceIconUrl(
											String(service.id_genserv),
										)}
									/>
								</View>
							</View>
