...
const HotelListItem = (props: Props) => {
  ...
  return (
    <View style={styles.container}>
      <View>
        <ImageComponent source={{uri: props.data.image}} style={styles.image} resizeMode="cover" />
        <Pressable style={styles.favotiteIconContainer} onPress={handleOnChangeFavorite}>
          <Animated.View style={{ transform: [{ scale: scaleValue }] }}>
            {props.data.favorite ? (
              <Image source={heart_icon} style={styles.favoriteIcon}/>
            ) : (
              <Image source={heart_outline_icon} style={styles.favoriteIconDisabled}/>
            )}
          </Animated.View>
        </Pressable>
      </View>
      <View style={styles.content}>
        <View style={styles.cardTitle}>
          <TextComponent weight="bold" size="18" numberOfLines={2}>{props.data.name}</TextComponent>
        </View>
