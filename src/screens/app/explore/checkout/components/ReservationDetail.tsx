...
import { parseDateStringToDate } from "@app/utils/dates.util";
...
const ReservationDetail = (props: Props) => {
	...
	const { currency } = useAppSelector(state => state.general);
			...
			<View style={styles.subSection}>
				<View style={styles.rowSpaceBetween}>
					...
					<TextComponent size="16" color="dark">
						...
						{`${generatePriceFormatted(props.purchaseTaxes, currency)}`}
					</TextComponent>
				</View>
				...
				<View style={styles.rowSpaceBetween}>
					...
					<TextComponent size="16" color="dark" weight="bold">
						{/* {`$${formatToCurrency(
							props.totalPurchaseAmount ?? calcTotalAmount(),
						)}`} */}

						{`${generatePriceFormatted(
							props.totalPurchaseAmount ?? calcTotalAmount(),
							currency,
						)}`}
					</TextComponent>
