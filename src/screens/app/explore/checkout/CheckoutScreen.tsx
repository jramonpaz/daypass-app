...
import { CommonActions, CompositeNavigationProp, NavigationProp, useFocusEffect, useNavigation } from '@react-navigation/native';
import { confirmPlatformPayPayment, isPlatformPaySupported, PlatformPay, useStripe } from '@stripe/stripe-react-native';
...
const CheckoutScreen = () => {
  ...
  useFocusEffect(
    useCallback(() => {
      if (error) {
        Alert.alert(
          t('checkout-screen-error-alert-title'),
          error,
          [
            {
  ...
  */
  const fetchPaymentSheetParams = useCallback(
    async () => {
      ...
      if (!!isAuth && !!userDetail) {
        ...
        const response = await dispatch(getShoppingCartPurchaseAnonymousService(payload));
  ...
  async function goToPaymentScreen (formValues:ISignInAnonymousFromData) {
    if (!hotelSelected || !hotelVenuePurchaseResponse || !searchSelectedPrediction || !shoppingCartPurchase) {
    ...
    if (isAuth && userDetail) {
      ...
      if (response) {
       ...
       const response = await dispatch(executePurchaseAsAnonymousService(payloadEcecutePurchace));
       if (response) {
          //  navigation.navigate('CHECKOUT_CONFIRM_SCREEN');
          navigation.dispatch(
            CommonActions.reset({
          ...
          <>
            ...
            <SectionItems title={t('checkout-screen-guest-purchase-title')}>
              <Controller
                ...
                name="name"
              />
              {errors.name && <TextComponent color="primary">{t('checkout-screen-field-required')}</TextComponent>}
              ...
              <Controller
                ...
                name="surname"
              />
              {errors.surname && <TextComponent color="primary">{t('checkout-screen-field-required')}</TextComponent>}
              ...
              <Controller
                ...
                name="mail"
              />
              {errors.mail && <TextComponent color="primary">{t('checkout-screen-field-required')}</TextComponent>}
