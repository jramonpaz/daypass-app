import { View, SafeAreaView, StyleSheet, StatusBar, Image, Pressable, FlatList, ListRenderItem, RefreshControl } from 'react-native';
...
const ListingOfDaypassScreen = () => {
  ...
  const handleFilterButtonPress = async (buttonIndex: 1 | 2 | 3) => {
    ...
    if (buttonIndex === 1) { // more-relevant
    ...
    } else if (buttonIndex === 2) { // upper-cost
      const filterPayload: IAPIExplorerSearch = {
        ...searchFilterData,
        orderby: 2,
      };
      ...
      dispatch(searchActions.setSearchFilterDataBackup(filterPayload));
      await dispatch(general_searchGeneral(filterPayload));
    } else if (buttonIndex === 3) { // lower-cost
      const filterPayload: IAPIExplorerSearch = {
        ...searchFilterData,
        orderby: 1,
      };
  ...
  const renderItem: ListRenderItem<IHotelListItem> = ({item}) => {
    return (
      <DaypassListItemCard
        hotelItem={item}
        onPress={() => handleOnSelectDaypass(item)}
      />
    );
  };
  ...
  return (
    <View style={styles.main}>
        ...
        <View style={styles.header}>
          <View style={styles.rowFilter}>
            ...
            <FilterComponent
              ...
              date={formatDate(searchFilterData?.date_search ? parseDateStringToDate(searchFilterData.date_search) : new Date())}
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <CardLabelComponent
            label={isLoading ? t('hotels-list-searching') : t('hotels-list-no_results')}
          />
        }
        refreshControl={<RefreshControl refreshing={isLoading} onRefresh={handleRefreshPage} />}
