      ...
      <View style={styles.cardDetail}>
        <Pressable style={styles.cardDetail} onPress={props.onPress}>
          <View style={styles.detailHeadContainer}>
            <TextComponent size="16" weight="bold">{hotelItem.name}</TextComponent>
            <View style={styles.rowDetail}>
              <Image
                ...
                resizeMethod="resize"
              />
              <TextComponent size="14" color="dark">{hotelItem.stars.toFixed(1)}</TextComponent>
              <TextComponent size="14" color="muted">{`${hotelItem.rating_descp}•(${hotelItem.reviews})`}</TextComponent>
            </View>

            <View style={styles.moodsRow}>
              {hotelItem.vibes.map(vibe => (
                <MoodButton mood={vibe[language as keyof typeof vibe]} key={vibe.id} />
              ))}
            </View>
          </View>
          ...
          {hotelItem.services.map((service, index) => {
            ...
            return (
              <LabelWithIcon
                ...
                iconComponent={IconComponent && <IconComponent fill={colors.muted}/>}
                theme="muted"
                size="small"
              />
            );
        ...
        >
          {hotelItem.tickets.map((ticket, index) => {
            ...
            if (ticket.isIncluded) {
              ...
              alertLabel = `${t('only')} ${ticket.remainingTickets} ${t('left')}`;
              type = 'yellow';
            }
