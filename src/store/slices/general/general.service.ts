...
import {
	ICurrency,
	ILanguageMock,
	IPrivacyPolicyQueryParams,
	ITermsConditionQueryParams,
} from "@app/types";
...
export const getGeneralPolPrivacy =
	(query: IPrivacyPolicyQueryParams = defaultQuery) =>
...
export const getGeneralTermsConditions =
	(query: ITermsConditionQueryParams = defTermsQuery) =>
	async (dispatch: Dispatch) => {
		try {
			dispatch(generalActions.clean());
			dispatch(generalActions.setIsLoading(true));
