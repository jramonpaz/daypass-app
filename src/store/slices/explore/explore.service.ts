...
export const general_getAllGeneralService = () => async (dispatch: Dispatch) => {
	try {
		...
		const url = `/general/getAllGeneralService?${createQueryParams(query)}`;
		const { data: response, status } = await axios.get(url);
		// 200 = Ok
		// 400 = BadRequest
		// 500 = InternalServerError
		if (status === 200) {
			dispatch(exploreActions.setSuccess(t("explore-service-general-services-loaded")));
			dispatch(exploreActions.setGeneralServicesList(response.data));
		} else {
			dispatch(exploreActions.setError(t("explore-service-general-services-error")));
			dispatch(exploreActions.setGeneralServicesList([]));
		}
	} catch (error) {
		console.error("Error getting all general services", error);
		dispatch(exploreActions.setSuccess(null));
		dispatch(exploreActions.setError(t("explore-service-general-services-error-try-again")));
	} finally {
...
export const general_searchGeneral =
	(payload: IAPIExplorerSearch) => async (dispatch: Dispatch) => {
		try {
			dispatch(exploreActions.clean());
			...
			if (status === 200) {
				...
				dispatch(
					exploreActions.setError(
						t("explore-service-general-search-error-invalid-data"),
					),
				);
			}
		} catch (error) {
			...
			dispatch(
				exploreActions.setError(t("explore-service-general-search-error-try-again")),
			);
		} finally {
			dispatch(exploreActions.setIsLoading(false));
...
export const general_searchGeneral_nextPage =
	(payload: IAPIExplorerSearch) => async (dispatch: Dispatch) => {
		try {
			dispatch(exploreActions.clean());
			dispatch(hotelActions.setIsLoading(true));
...
export const general_searchGeneral_refresh =
	(payload: IAPIExplorerSearch) => async (dispatch: Dispatch) => {
		try {
			dispatch(exploreActions.clean());
			dispatch(hotelActions.setIsLoading(true));
...
export const general_searchGeneralCount =
	(payload: IAPIExplorerSearch, getMinMax: boolean) => async (dispatch: Dispatch) => {
		try {
			dispatch(exploreActions.clean());
			dispatch(hotelActions.setIsLoading(true));
			dispatch(exploreActions.setIsLoading(true));
...
export const general_getNearbyHotels =
	(request: IHotelNearbyRequest) => async (dispatch: Dispatch) => {
		try {
			dispatch(exploreActions.clean());
			dispatch(exploreActions.setIsLoading(true));
